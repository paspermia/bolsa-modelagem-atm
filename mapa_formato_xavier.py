...
# 1. DADOS DE ENTRADA
# Utilizando os mesmos valores do INMET e a diferença calculada para o BR-DWGD
dados = {
    'Estacao': ['Bento Gonçalves', 'Caxias do Sul', 'Santa Maria'],
    'Lat': [-29.16, -29.16, -29.70],
    'Lon': [-51.53, -51.18, -53.70],
    'INMET_mm': [74.40, 76.20, 85.80],  # Chuva real para ditar o tamanho da bolha
    'Diferenca': [-3.4, 11.0, 19.3]     # Viés do BR-DWGD para ditar a cor
}
...
cbar_chuva.set_label('Viés do BR-DWGD (mm): Positivo=Superestimou | Negativo=Subestimou', fontsize=10, fontweight='bold')
...
for idx, row in df_mapa.iterrows():
    ...
    # Prevenção de colisão entre Caxias e Bento Gonçalves
...
ax.set_title('Desempenho do Modelo BR-DWGD vs Estações INMET\nAcumulado 24h (Setembro 2023)', 
             fontsize=14, fontweight='bold', pad=15)
ax.set_xlabel('Longitude', fontsize=10)
ax.set_ylabel('Latitude', fontsize=10)
ax.grid(color='grey', linestyle=':', linewidth=0.5, alpha=0.5, zorder=0)

plt.savefig('mapa_diferenca_xavier_publicacao.png', dpi=300, bbox_inches='tight', transparent=False)
plt.show()